...
public class JsonPersistence<T> implements ResourceTreePersistence<T> {
    ...
    public JsonPersistence(File jsonFile, Class<T> type, TypeAdapter<T> typeAdapter) {
        this.jsonFile = jsonFile;
    ...
    public void load(ResourceTree<T> resourceTree) {
        ...
        try (Reader reader = new FileReader(jsonFile)) {
            List<TreeModel<T>> rootModels = gson.fromJson(reader, new TypeToken<List<TreeModel<String>>>(){}.getType());
            reader.close();
